...
**Key Definitions** (from `MainTheorem.lean`):

- **φ** (golden ratio): `φ = (1 + √5)/2`
- **r_crit** (critical radius): `r_crit = √(3 + φ)`
- **genA_n n r**: N-fold generator rotating by -2π/n about (-1), identity outside left disk
- **genB_n n r**: N-fold generator rotating by -2π/n about (+1), identity outside right disk
- **TwoDiskCompoundSymmetryGroup n hn r**: Subgroup of `Equiv.Perm ℂ` generated by A_n and B_n
- **GG5_At_Critical_radius**: `TwoDiskCompoundSymmetryGroup 5 (by norm_num) r_crit`
...
- **MainTheorem.lean** contains essential definitions and the theorem statement
- **ProofOfMainTheorem.lean** provides the proof
- **Definitions/** contains all definitions with axioms (human-reviewable, ~1500 lines)
- **Proofs/** contains only lemmas and theorems (machine-verified, ~8500 lines)
- Uses only standard axioms

**Key architectural decision**: MainTheorem.lean contains core definitions (genA_n, genB_n, TwoDiskCompoundSymmetryGroup, etc.) to make the theorem statement self-contained and readable. Definitions/GroupTheory.lean imports MainTheorem and provides proofs for the stated axioms. KMVerify has been updated to allow this pattern.
...
--------------------------------------------------------------------------------
  ...
  Definitions/                8 files     1336 lines
  Proofs/                     19 files    8553 lines
  MainTheorem.lean                        161 lines
  ProofOfMainTheorem.lean                 122 lines
--------------------------------------------------------------------------------
  REVIEW BURDEN: 1497 lines (Definitions + MainTheorem)
  TOTAL: 10172 lines (14% requires review)

CHECKS
--------------------------------------------------------------------------------
  [PASS] Structure
  [PASS] Axioms (genA_n_bijective, genB_n_bijective proven in Definitions/)
  [PASS] Transparency
  [PASS] Completeness
...
```text
TDCSG/
  MainTheorem.lean            # Core definitions + theorem statement (161 lines)
                              # Contains: φ, r_crit, genA_n, genB_n,
                              #           TwoDiskCompoundSymmetryGroup, GG5_At_Critical_radius
  ProofOfMainTheorem.lean     # Complete proof of main theorem (122 lines)

  Definitions/                # All definitions (8 files, 1336 lines)
    Core.lean                 # φ, r_crit, Generator, Word, ζ₅ (zeta5), Circle defs
    Geometry.lean             # Disk geometry, rotations, geometric primitives
    GroupAction.lean          # genA, genB (5-fold), applyGen, applyWord, orbit
    GroupTheory.lean          # Proofs of genA_n_bijective, genB_n_bijective
                              # 5-fold infrastructure (genA_perm, genB_perm)
  ...
  Proofs/                     # Supporting lemmas (19 files, 8553 lines)
