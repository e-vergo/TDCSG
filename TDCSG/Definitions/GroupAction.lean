...
## Bijectivity proofs
- `genA_n_bijective`, `genB_n_bijective` : N-fold generators are bijections
...
open Real Complex
open scoped Circle
-- φ and r_crit are now in TDCSG.Definitions (from Core.lean)
-- genA_n and genB_n are now in TDCSG.Definitions (from Geometry.lean)
...
/-! ## N-fold Generator Bijectivity Proofs

This section contains the bijectivity proofs for the generalized N-fold generators
`genA_n` and `genB_n`. These proofs establish that the generators define valid
group elements (bijections of ℂ).

The key insight is that N rotations by -2π/N returns to the identity, so
`genA_n n` and `genB_n n` have order n, and hence are bijections with explicit
inverses given by `f^[n-1]`.
-/

/-! ### Circle Exponential Periodicity

These lemmas establish the fundamental periodicity property: rotating by -2π/n
a total of n times yields the identity transformation. This is the key to proving
the generators have order n.
-/
...
/-- n rotations by -2π/n = identity for n ≥ 1 (general order-n property). -/
...
/-! ### Disk Preservation Under Rotation

Rotations about the disk centers preserve disk membership.
-/
...
/-! ### Generator Behavior Inside and Outside Disks -/
...
/-! ### Order-n Property: Generators Have Period n

The key structural fact: applying genA_n or genB_n exactly n times yields the identity.
This is what makes them order-n elements in the group.
-/
...
/-! ### Bijectivity of Generators

Using the order-n property, we prove that genA_n and genB_n are bijections.
Since f^n = id, we can construct explicit inverses: f^[n-1] is the inverse of f.
-/

/-- The bijectivity of genA_n for n ≥ 1.

    Proof strategy: Since (genA_n)^n = id, we have f^[n-1] as the inverse of f,
    which immediately implies bijectivity. -/
...
/-- The bijectivity of genB_n for n ≥ 1.
    ...
    Proof strategy: Since (genB_n)^n = id, we have f^[n-1] as the inverse of f,
    which immediately implies bijectivity. -/
