...
This file contains theorems about interval exchange transformations (IETs), which are a fundamental
class of piecewise isometries on the unit interval. An IET partitions [0,1] into finitely many
...
- They connect to Teichmuller theory and flat surfaces

## Main results

- `interval_nonempty`: Domain intervals are nonempty
...
- `domainLeft_mono`: Left endpoints are monotone
- `domainRight_le_domainLeft_of_lt`: Contiguity of intervals
- `interval_injective`: The interval function is injective
- `intervalContaining_unique`: Interval containing a point is unique
- `mem_intervalContaining`: The chosen interval contains the point
...
theorem intervals_cover : ⋃ i, iet.interval i = Ico 0 1 := by
  ...
  · -- If 0 <= x < 1, then x is in some interval
    ...
    -- whether x in interval i. Since x in [0,1) and intervals cover [0,1),
...
lemma mem_intervalContaining (x : ℝ) (hx : x ∈ Ico 0 1) :
    x ∈ interval iet (intervalContaining iet x hx iet.intervals_cover) :=
...
lemma intervalContaining_unique (x : ℝ) (hx : x ∈ Ico 0 1) (i : Fin n) (hi : x ∈ interval iet i) :
    intervalContaining iet x hx iet.intervals_cover = i := by
  have h_mem := mem_intervalContaining iet x hx
  by_contra h_ne
  have h_disj := iet.intervals_disjoint (Set.mem_range_self (intervalContaining iet x hx iet.intervals_cover))
                                       (Set.mem_range_self i)
                                       (by intro h_eq; exact h_ne (iet.interval_injective h_eq))
  have : x ∈ interval iet (intervalContaining iet x hx iet.intervals_cover) ∩ interval iet i := ⟨h_mem, hi⟩
