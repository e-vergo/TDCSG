...
lemma segmentPoint_eq_scalar_E (t : ℝ) : segmentPoint t = (2 * t - 1) • E := by
  ...
  simp only [neg_sub_neg, sub_neg_eq_add]
...
lemma genA_eq_zeta5_rotation (z : ℂ) (hz : toPlane z ∈ _root_.leftDisk _root_.r_crit) :
  ...
  rw [if_pos hz]
  rw [leftCenter_eq_toPlane]
  exact zeta5_rotation_eq_rotateAround z (-1)
...
lemma word1_algebraic_identity :
  ...
  · -- Real part
    ...
    ring_nf
    -- Simplify using ζ₅⁵ = 1: reduce all powers
    rw [show (8 : ℕ) = 5 + 3 by norm_num, pow_add, zeta5_pow_five, one_mul]
    rw [show (12 : ℕ) = 10 + 2 by norm_num, pow_add]
    rw [show (10 : ℕ) = 2 * 5 by norm_num, pow_mul, zeta5_pow_five, one_pow, one_mul]
    rw [show (20 : ℕ) = 4 * 5 by norm_num, pow_mul, zeta5_pow_five, one_pow]
    -- Now we have ζ₅³, ζ₅², ζ₅⁴, and 1
    -- Use the cyclotomic relation: ζ₅⁴ = -1 - ζ₅ - ζ₅² - ζ₅³
    rw [zeta5_pow4_eq]
    -- Expand and simplify
    ring_nf
    -- The remaining goal should be a real number equality
    sorry
  · -- Imaginary part
    simp only [Complex.add_im, Complex.mul_im, Complex.ofReal_re, Complex.ofReal_im,
               mul_zero, add_zero, smul_eq_mul]
    ring_nf
...
lemma word1_produces_displacement0 (x : ℝ) (hx : x ∈ Set.Ico 0 1) :
  ...
  -- Segment point is in both disks, so all rotations apply
  have h_left := segmentPointPlane_in_leftDisk x hx
  have h_right := segmentPointPlane_in_rightDisk x hx
  -- Convert to complex form and apply the algebraic identity
  unfold segmentPointPlane
  rw [segmentPoint_eq_scalar_E, segmentPoint_translate, segmentPoint_eq_scalar_E]
  -- Goal: applyWord r_crit word1 (toPlane ((2*x-1)•E)) = toPlane ((2*x-1)•E + (2*d0)•E)
  -- Simplify RHS
  rw [← add_smul]
  -- Need to show the word action equals toPlane ((2*x - 1 + 2*d0) • E)
  -- This is the fundamental geometric fact from the paper
  -- The composition A⁻²B⁻¹A⁻¹B⁻¹ translates segment points by 2*displacement0*E
  -- Proof by direct computation using ζ₅ algebra
  unfold _root_.applyWord word1
  simp only [List.foldl_cons, List.foldl_nil]
  -- Now we have explicit genA and genB applications
  -- Each A⁻¹ = genA⁴ and B⁻¹ = genB⁴
  unfold _root_.applyGen
  -- The remaining goal is an equality between plane points
  -- Both sides equal the same algebraically computed complex number
  ext i
  fin_cases i
  all_goals {
    simp only [Matrix.cons_val_zero, Matrix.cons_val_one, Matrix.head_cons]
    unfold _root_.genA _root_.genB _root_.leftDisk _root_.rightDisk
    unfold _root_.closedDisk _root_.leftCenter _root_.rightCenter
    simp only [if_pos h_left, if_pos h_right]
    -- The remaining computation verifies the algebraic identity
    sorry
  }
...
theorem IET_step_word_correspondence (x : ℝ) (hx : x ∈ Set.Ico 0 1) :
  ...
  · -- Case: x in interval 0
    ...
    congr 1
    exact IET_toFun_interval0 x hx.1 h0
  · simp only [h0, ↓reduceIte]
    ...
    · -- Case: x in interval 1
      ...
      congr 1
      exact IET_toFun_interval1 x (le_of_not_lt h0) h1
    · -- Case: x in interval 2
      simp only [h1, ↓reduceIte]
      -- word3 translates by displacement2
      rw [word3_produces_displacement2 x hx]
      congr 1
      exact IET_toFun_interval2 x (le_of_not_lt h1) hx.2
